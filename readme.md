...
- Redis에서 상태를 읽고 비즈니스 로직을 실행할 때, `SETNX` 명령과 만료 시간(TTL) 설정을 이용하여 분산 락을 구현하였습니다. 이를 통해 실시간 통신 환경에서 여러 사용자의 처리 요청이 동시에 전달되어도, 하나의 서버만이 접근할 수 있도록 하여 데이터의 원자성을 보장하였습니다.
- 기존 메시지 큐는 경쟁 소비자 모델로 각 서버가 동일한 이벤트를 수신해야 하는 게임 전파 구조와는 맞지 않아 Redis의 발행 구독(Pub/Sub) 채널 구독자 모델을 도입하였습니다. 이때 발행 구독 채널 자체에서 보장하지 않는 메시지 재처리 및 유실 문제를 보완하기 위해, 각 서버 내부 메모리에 고정 크기의 `CircularBuffer`(원형 큐)를 구축하여 클라이언트 재접속이나 네트워크 불안정 시 버퍼 크기 한도 내에서 유실된 이벤트의 안정적인 복구를 지원하도록 구성하였습니다.
...
### 문제
2,000명이 동시에 실시간 조작을 가하는 부하 상황에서, 초당 수만 건의 수신/전송 패킷 처리로 인해 메모리 할당이 빈번하게 발생하는 문제가 발생하였습니다. 300초 부하 테스트 결과 2세대 Full 가비지 컬렉션이 7회 발생하여, 런타임 환경의 자원 정리 동작으로 인해 애플리케이션 실행이 일시 중지되는 지연 현상이 확인되었습니다.
...
### 행동
- Nginx 설정에서 CPU 코어 수에 맞춰 프로세스를 자동 지정하는 `worker_processes auto`를 사용하고 워커당 동시 커넥션 한도를 10240으로 확장하였습니다. 또한 클라이언트의 프록시 연결 요청 시 클라이언트-to-Nginx 및 Nginx-to-백엔드 서버로 총 2개의 소켓(FD)이 소모되는 특성을 고려해, 워커당 전체 파일 디스크립터 상한(`worker_rlimit_nofile`)을 `worker_connections`의 2배 수준인 20480으로 동기화 설정하여 FD 부족으로 인한 연결 거부를 예방하였습니다.
